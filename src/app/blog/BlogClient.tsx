...
export default function BlogClient({ initialBlogs = [] }: { initialBlogs?: any[] }) {
  ...
  const handleCardImageLoad = (index: number, e: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    // If very wide (> 1.8), align to left-top so start of title/diagram isn't cropped
    if (aspect > 1.8) {
      setCardPositions((prev) => ({ ...prev, [index]: "object-left-top" }));
    } else if (aspect < 1.62) {
      // If tall, keep top aligned
      setCardPositions((prev) => ({ ...prev, [index]: "object-top" }));
    }
  };
  ...
  const currentFeatured = activePosts.slice(0, 4);
              ...
              {currentFeatured.map((post, i) => (
                ...
                >
                  <ImgBox
                    ...
                    alt={post.title}
                    fill
                    imgClassName={cn(cardPositions[i] || "object-center")}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/40 to-transparent p-5 sm:p-6 flex flex-col justify-end text-white z-10">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3 text-gray-400 text-[9px] sm:text-[10px]">
                        <span>
                          <CalendarIcon />
                          {post.date}
                        </span>
                      ...
                      <div className="text-white text-[9px] sm:text-[10px] font-bold flex items-center gap-1 group-hover:text-secondary p-0 min-h-0 border-0 transition-colors">
                        Read More{" "}
...
interface MobileCarouselProps {
  ...
  autoSlideInterval?: number; // Auto-slide time in ms (default 3500)
}

function MobileCarousel({ children, className, autoSlideInterval = 3500 }: MobileCarouselProps) {
  ...
  }, [handleNext, autoSlideInterval, childrenArray.length]);
