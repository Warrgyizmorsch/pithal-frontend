...
import type {
  ProductStat,
  ProductStatsSectionData,
} from "@/data/products/productDetailTypes";
...
function PerformanceStrip({ text }: { text: string }) {
  ...
  const [betweenHighlights, afterResults = ""] =
    afterPerformance.split(results);
      ...
      <div className="relative -mt-1 w-full border-y border-border/90 bg-white/96 py-7 shadow-[0_10px_24px_rgba(3,27,64,0.08)] lg:py-8">
        ...
        <div className="mx-auto w-full max-w-[1440px] px-4 sm:px-6 lg:px-8">
          <header className="relative mx-auto mb-6 max-w-4xl text-center">
            <h2 className="headline text-primary text-[38px] sm:text-[2.5rem] leading-[1.02] sm:leading-tight">
              <span className="block sm:inline">{section.title}</span>
              <span className="block sm:inline text-secondary sm:before:content-['\00a0']">{section.highlight}</span>
            </h2>
          ...
          <div className="relative grid gap-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            {stats.map((stat, index) => (
              ...
              >
                ...
                <h3 className="text-[13px] font-black uppercase tracking-[0.02em] text-primary sm:text-sm">
                  {stat.label}
                </h3>
                ...
                {stat.label === "Application" ? (
                  ...
                  <p className="headline mt-3 text-[clamp(1.8rem,2.6vw,2.4rem)] leading-none text-secondary">
                    {stat.value}
                  </p>
                )}
                {stat.unit && (
                  <p className="mt-1.5 text-[13px] font-medium leading-snug text-text-dark sm:text-sm">
                    {stat.unit}
                  </p>
                )}
                {stat.description && (
                ...
                {index < stats.length - 1 && (
                  <span
                    aria-hidden
                    className="absolute -right-[11px] top-1/2 z-10 hidden h-7 w-5 -translate-y-1/2 bg-secondary [clip-path:polygon(0_0,55%_50%,0_100%,45%_100%,100%_50%,45%_0)] xl:block"
                  />
                )}
              </Card>
            ))}
          ...
          {section.cta && (
            <div className="mt-5 flex justify-center">
              <ProductCtaButton
                className="min-h-11 w-full sm:w-auto justify-center gap-2.5 rounded px-6 text-xs font-black uppercase tracking-wider"
                cta={section.cta}
                iconSize={16}
              />
            </div>
