...
const videoProcessSteps = [
  {
    ...
    text: "Raw material is loaded into the hopper by dump trucks or loaders.",
    video: "/workvideo/magnific_animate-this-image-as-the_LUm7UMHswO.mp4"
  },
  {
    number: "02",
    title: "Feed Hopper",
    text: "Feeder hopper regulates the continuous and uniform flow of materials.",
    video: "/workvideo/magnific_animate-this-image-as-the_SOfPNJrUb8.mp4"
  },
  {
    number: "03",
    title: "Primary Crushing",
    text: "Jaw crusher reduces large rocks into smaller, manageable sizes.",
    video: "/workvideo/magnific_animate-this-image-as-the_vunFBBia47.mp4"
  },
  {
    number: "04",
    title: "Secondary Crushing",
    text: "Further size reduction using cone crusher for consistent output.",
    video: "/workvideo/magnific_animate-this-image-as-the_WMwpnNgcXe.mp4"
  },
  {
    number: "05",
    title: "Screening",
    text: "Vibrating screens separate material into different sizes.",
    video: "/workvideo/magnific_animate-this-image-as-the_1soSjUvr4r.mp4"
  },
  {
    number: "06",
    title: "Conveyor Belt",
    text: "Carries materials to the desired location.",
    video: "/workvideo/magnific_animate-this-image-as-a-c_kLBgSFq16B.mp4"
  },
  {
    number: "07",
    title: "Final Output",
    text: "High-quality aggregates ready for various applications.",
...
export function ProcessTimeline({
  ...
  const handleTouchEnd = () => {
    ...
    if (isLeftSwipe) {
      setActiveIndex((prev) => (prev + 1) % videoProcessSteps.length);
    } else if (isRightSwipe) {
      setActiveIndex((prev) => (prev - 1 + videoProcessSteps.length) % videoProcessSteps.length);
    }
  };
  ...
  return (
    <section className={`relative bg-white ${sectionPadding}`}>
        ...
        <div className="hidden sm:grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7 xl:items-stretch xl:gap-4 mt-12">
          {videoProcessSteps.map((step, index) => {
            ...
            return (
              <div
                key={step.number}
                className="relative min-w-0 flex cursor-pointer"
              ...
              >
                ...
                >
                  <div className="mb-3 flex items-start gap-2">
                    <div
                      className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-[14px] font-black leading-none text-secondary transition-colors duration-300 ${
                    ...
                    <div className="flex min-w-0 flex-1 flex-col justify-center">
                      ...
                      <p className="text-[10px] leading-[1.4] text-text-dark xl:text-[9px]">
                        {step.text}
                      </p>
                    </div>
                  </div>
                  <div className="relative mt-auto h-56 w-full overflow-hidden rounded-xl bg-white min-[420px]:h-64 sm:h-72">
                    {mounted ? (
                      <video
                        ref={(el) => {
      ...
      <div 
        className="sm:hidden relative w-full overflow-hidden py-4 mt-8 cursor-grab active:cursor-grabbing"
        ...
        >
          {videoProcessSteps.map((step, index) => {
            ...
            return (
              ...
              >
                <article
                  className={`relative flex h-[380px] min-w-0 flex-col justify-end rounded-2xl border-2 overflow-hidden p-3 transition-all duration-300 ${
                    isActive
      ...
      <Container>
        ...
        <div className="flex justify-center items-center gap-3 mt-6 text-secondary">
