...
export function ProductCard({
  ...
  return (
    <article className="image-hover lift flex h-full flex-col overflow-hidden rounded border border-border bg-white p-4 sm:p-5 xl:p-6">
      <div className="mb-5 flex flex-col items-center justify-between min-h-[3.25rem] sm:mb-6">
      ...
      <div className="relative mb-4 h-64 overflow-hidden rounded bg-bg-light sm:mb-6 sm:h-48">
        <Image alt={title} className="object-contain" fill sizes="(max-width: 767px) calc(100vw - 2.5rem), (max-width: 1279px) 50vw, 25vw" src={image} />
      </div>
      <div className="flex flex-grow gap-3 rounded bg-white p-2 text-white sm:gap-4 sm:p-4">
        ...
        <p className="text-sm leading-6 text-text-muted flex-grow min-h-[4.5rem] line-clamp-3">{description}</p>
      </div>
